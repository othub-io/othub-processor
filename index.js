    ...
    if (data.network === "otp::testnet") {
      await testnet_dkg.asset
        ...
        .catch(async (error) => {
          console.log(error);
          console.log(
            `${wallet_array[index].name} wallet ${wallet_array[index].public_key}: Transfer failed. Retrying in 3 minutes...`
          );
...
async function uploadData(data) {
  try {
    ...
    console.log(JSON.parse(data.txn_data))
...
async function getPendingUploadRequests() {
  try {
    ...
    for (i = 0; i < network_array.length; i++) {
      sqlQuery =
        "select txn_id,progress,approver,network,txn_data,keywords,epochs,updated_at,created_at,receiver FROM txn_header where progress = ? and network = ? and request = 'Create-n-Transfer' ORDER BY created_at ASC LIMIT 1";
